...
class CRF(object):
    ...
    def forwardBackward(self, matrix, labels = None):
        ...
        for j in range(seqlen - 1, -1, -1):
            ...
            for st in allowed:
                ...
                if j == 0:
                    self['BEGIN'].fw[0][0] += self[st].bw[j][0] * numpy.exp(self['BEGIN'].getTransition(st)) * self[st].scores[j]
                    if clamped and st in allowed_c:
                        self['BEGIN'].fw[0][1] += self[st].bw[j][1] * numpy.exp(self['BEGIN'].getTransition(st)) * self[st].scores[j]
    ...
    def predict(self, sample, algo = 'viterbi', be = True, prob = False):
        self.algo = algo
        self.be = be
        ret = []
        for el in sample:
            p = self.doPrediction(el[0], prob = prob)
            ret.append(p)
        return ret

    def doPrediction(self, matrix, prob = False):
        ret = None
        if self.algo == 'viterbi':
            ret = self.viterbi(matrix)
        elif self.algo == 'posterior-viterbi-sum':
            ret = self.posteriorViterbi(matrix, prob = prob)
        elif self.algo == 'posterior-viterbi-max':
            ret = self.posteriorViterbi(matrix, suml = False, prob = prob)
        else:
            pass
        return ret
